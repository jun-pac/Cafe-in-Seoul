...
- **검색 의도별 collection 페이지** — broad 쿼리("서울 늦게까지 하는 카페", "best cafes to work in Seoul")를 먹는 대표 문서. `/cafes/<key>` + `/en/cafes/<key>`. **속성 8종**(`best-study-cafes-seoul`·`late-night`·`24-hour`·`great-view`·`power-outlets`·`quiet`·`affordable`·`spacious`) + **지역 자동**(카페 4곳 이상인 서울 구 = `gangnam`·`mapo`·`yeongdeungpo`·`용산`… — `SEOUL_GU` 로마자 slug). 각 페이지 = 리드 + 비교표 + 랭킹 카드(사진+요약) + 선정기준 + 상호링크 + `CollectionPage`/`ItemList`/`BreadcrumbList` JSON-LD. **모두 서울 한정**(`isSeoul` = 주소가 "서울"로 시작 — 비서울 카페 경주·부산·횡성 등은 개별 페이지만, "서울 X" 리스트엔 안 섞임; 부산/인천 중구 같은 동명 구도 분리). 지역명은 전체 '구'(중구는 "중" 아님). **지역×조건 조합**(`/cafes/<hood>/<attr>`, 예 `영등포구 뷰 좋은 카페`)은 매칭 **3곳 이상일 때만** 생성(미만 404), 허브엔 없고 attr/hood 페이지 상호링크 + sitemap으로 노출. 라우트는 `/cafes/:slug`를 공유하되 collection이 아니면 상세 핸들러로 폴백(상세 slug는 `-8hex`로 끝나 충돌 없음). 4곳 미만 구는 404. faceted-nav URL 폭발 없이 실제 검색 의도만 승격.
