...
- "우천 접근성" 정보는 **지하철역과 지하통로로 직결돼 비 안 맞고 갈 수 있다**는 접근성 장점을 뜻한다("비 오는 날 작업하기 좋다"는 의미 아님). 이 정보가 주어졌을 때만 그 장점으로 언급하라. 주어지지 않았으면 **지하철 연결 여부를 네가 지어내 언급하지 마라** — 특히 지하철이 없는 지역(지방 등)에 "지하철과 연결되지 않아 접근성이 나쁘다" 같은 말은 절대 금지. 접근성·위치의 장단점 자체는 '직접 쓴 카공 총평'에 적혀 있으면 그 내용을 근거로 자연스럽게 반영해도 된다(지하철을 억지로 끌어들이지 말고, 총평에 적힌 실제 사실대로).
