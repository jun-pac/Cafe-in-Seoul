...
- **UI 정리**: "뷰 맛집" 완전 제거 → "주인장 추천 카페"/"주인장 추천 명소"(짧게 "카페"/"명소"). 카테고리 토글 헤더 인라인, 필터 한 줄, 아이스아메리카노→"아아", 면적/콘센트 자체라벨 드롭다운(+ⓘ 기준 툴팁), 우천시 우산 아이콘, 모바일 로그인 중앙정렬, 지도 스크린샷(`preserveDrawingBuffer`).
